---
name: coder
description: 一切开发任务的总入口——只要任务涉及编写、修改、审查、调试或重构代码（不限语言和文件类型），必须先加载本技能。任务开始处理代码前的第一个动作必须是运行 scripts/query.js 查询相关组件/函数（硬性，唯一豁免见 SKILL.md），先查后写。当前深度规范内置 Vue 3 与 TypeScript（.vue 使用 SFC 结构规范；.ts 复用相同的先查后写、类型优先、最小改动流程），其他语言按通用开发流程处理，并通过范式积累机制 references/<language>.md 随开发经验持续沉淀新语言/框架的最佳实践。铁律（每次任务必须遵守，优先级最高）：禁止过度设计与过度开发，永远用最简方式实现当前需求；发现逻辑 bug 时先完成当前开发，再与用户确认是否修复。首次在项目中使用时先询问建库范围（全量扫库 / 只建当前需求相关 / 本次跳过）。索引超过 300 条自动启用 SQLite 模式。Vue 复杂场景再分派到 vue3-project-init、vue3-dev 或 vue3-deps。
---
...
## 查询优先（硬性，每个任务开始时的第一个动作）

**规则：每次编程任务开始处理代码前，必须先运行查询脚本**——这是任务的第一步，不是可选项：
...
1. **查询（硬性第一步）**：任务涉及哪些组件/函数名，先逐个运行 `query.js` 查询——写前查同名避免重复，改前查符号拿行号与依赖，查错前查符号定位实现。
2. 确认任务涉及的文件后缀和项目根目录。
3. 相似度检查：需要新建的组件/函数若与已有实现（查询命中结果）相似度 ≥80%（功能、入参、交互、场景 4 项中 3 项一致），先与用户确认是"调整已有"还是"新建"，禁止直接绕过已有实现另起炉灶。
...
13. 源码变更后执行 `query.js --refresh` 更新索引（改动涉及新增/删除/移动文件时必须，改文件内容可选）。项目已有 `.docs` 时可遵守其既有任务日志约定，但本技能不强制创建或维护 `.docs` 全量清单。
